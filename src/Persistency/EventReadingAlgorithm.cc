...
StatusCode EventReadingAlgorithm::Initialize()
{
    if (!m_geometryFileName.empty())
    {
        ...
        if (BINARY == geometryFileType)
        {
            ...
            PandoraReturnOnError(fileReader.ReadGeometry());
        }
    ...
    {
        PandoraReturnOnError(this->ReplaceEventFileReader(m_eventFileName));
        PandoraReturnOnError(m_pEventFileReader->GoToEvent(m_skipToEvent));
    }
...
StatusCode EventReadingAlgorithm::Run()
{
    if ((nullptr != m_pEventFileReader) && !m_eventFileName.empty())
    {
        ...
        PandoraReturnOnError(PandoraContentApi::RepeatEventPreparation(*this));
    }
...
void EventReadingAlgorithm::MoveToNextEventFile()
{
    ...
    PandoraThrowOnError(this->ReplaceEventFileReader(m_eventFileName));
...
StatusCode EventReadingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    {
        PandoraReturnOnErrorExcept(XmlHelper::ReadValue(xmlHandle, "GeometryFileName", m_geometryFileName), STATUS_CODE_NOT_FOUND);
    }
    ...
    {
        PandoraReturnOnErrorExcept(XmlHelper::ReadVectorOfValues(xmlHandle, "EventFileNameList", m_eventFileNameVector), STATUS_CODE_NOT_FOUND);
    }
    ...
    {
        PandoraReturnOnErrorExcept(XmlHelper::ReadValue(xmlHandle, "SkipToEvent", m_skipToEvent), STATUS_CODE_NOT_FOUND);
    }
    ...
    PandoraReturnOnErrorExcept(XmlHelper::ReadValue(xmlHandle,
        "SkipToEvent", m_skipToEvent), STATUS_CODE_NOT_FOUND);
