    ...
    const ObjectFactory<object_creation::CaloHit::Parameters, object_creation::CaloHit::Object> &factory)
{
    ...
    try
    {
        PandoraThrowOnError(factory.Create(parameters, pCaloHit));
        ...
        PandoraThrowOnError(this->Modifiable(pCaloHit)->SetPseudoLayer(pseudoLayer));
    ...
    const CaloHit *&pDaughterCaloHit2, const ObjectFactory<object_creation::CaloHitFragment::Parameters, object_creation::CaloHitFragment::Object> &factory)
{
    ...
    PandoraReturnOnError(factory.Create(parameters1, pDaughterCaloHit1));
    ...
    PandoraReturnOnError(factory.Create(parameters2, pDaughterCaloHit2));
    ...
    {
        PandoraReturnOnError(m_pCurrentReclusterMetadata->GetCurrentCaloHitMetadata()->Update(caloHitReplacement));
    }
    else
    {
        PandoraReturnOnError(this->Update(caloHitReplacement));
    }
    ...
    const CaloHit *&pMergedCaloHit, const ObjectFactory<object_creation::CaloHitFragment::Parameters, object_creation::CaloHitFragment::Object> &factory)
{
    ...
    PandoraReturnOnError(factory.Create(parameters, pMergedCaloHit));
    ...
    const std::string &originalReclusterListName)
{
    ...
    PandoraReturnOnError(this->CreateTemporaryListAndSetCurrent(pAlgorithm, clusterList, caloHitListName));
    ...
    PandoraReturnOnError(m_pCurrentReclusterMetadata->CreateCaloHitMetadata(pCaloHitList, caloHitListName,
...
StatusCode CaloHitManager::PrepareForClustering(const Algorithm *const pAlgorithm, const std::string &newReclusterListName)
{
    ...
    std::string caloHitListName;
    PandoraReturnOnError(InputObjectManager<CaloHit>::CreateTemporaryListAndSetCurrent(pAlgorithm, caloHitList,
        caloHitListName));
    CaloHitList *const pCaloHitList = m_nameToListMap[caloHitListName];
...
StatusCode CaloHitManager::EndReclustering(const Algorithm *const /*const pAlgorithm*/, const std::string &selectedReclusterListName)
{
    ...
    PandoraReturnOnError(m_pCurrentReclusterMetadata->ExtractCaloHitMetadata(selectedReclusterListName,
    ...
    if (--m_nReclusteringProcesses > 0)
    {
        ...
        PandoraReturnOnError(pCurrentCaloHitMetaData->Update(*pSelectedCaloHitMetaData));
    }
    else
    {
        m_pCurrentReclusterMetadata = nullptr;
        PandoraReturnOnError(this->Update(*pSelectedCaloHitMetaData));
    }
...
StatusCode CaloHitManager::Update(const CaloHitReplacement &caloHitReplacement)
{
    ...
    {
        PandoraReturnOnError(this->Update(mapEntry.second, caloHitReplacement));
    }
